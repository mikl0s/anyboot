...
*   **UI-1:** As a user, I want the option to use either a graphical interface (via a web browser) or a text-based interface (via Lynx) to control AnyBoot.
...
*   **UI-6:** As a user, I want the text-based interface to provide access to all core functionalities available in the graphical interface.
...
*   **Consistency:** The experience in both GUI (Chromium) and Text (Lynx) modes should be functionally equivalent, leveraging the single Next.js application base.
...
*   Both GUI (Chromium) and Text (Lynx) modes are functional for the core workflow.
